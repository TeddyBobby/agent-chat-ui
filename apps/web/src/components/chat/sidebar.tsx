...
export function Sidebar({ conversations, activeId, runningConvIds, onSelect, onArchive, onRestore, onDelete, onNewChat }: SidebarProps) {
  const { dark, toggle } = useTheme();
  const isStreaming = runningConvIds.size > 0;
      ...
      <div className="flex-1 overflow-y-auto px-2 py-1 space-y-0.5">
        {(view === 'active' ? activeConvs : archivedConvs).length === 0 ? (
          ...
          (view === 'active' ? activeConvs : archivedConvs).map((conv) => {
            ...
            const isRunning = runningConvIds.has(conv.id);
